...
public class CreativeViewController: UIViewController, ARSCNViewDelegate, SCNPhysicsContactDelegate, PlaygroundLiveViewSafeAreaContainer {
    ...
    var ballShape: BallType!
    var ballSize: CGFloat!
    var ballSpeed: Float!
    var blockShape: BlockType!
    var amountOfBlocks: (Int, Int)!
    
    public convenience init(_ ballShape: BallType, _ ballSize: CGFloat, _ ballSpeed: Float, _ blockShape: BlockType, _ amountOfBlocks: (Int, Int)) {
    ...
    func launchBall() {
        ...
        sceneView.scene.rootNode.addChildNode(BallNode(size: ballSize, type: ballShape, camera: sceneView.pointOfView!))
    }
    ...
    func placeNodes() {
        generateWall()
        BlockNode.generate(type: blockShape, gridSize: amountOfBlocks.0, lengh: amountOfBlocks.1)
        setBorder()
    ...
    func gameover() {
        ...
        WallNode.changeColor(.red)
        PlaygroundPage.current.assessmentStatus = .pass(message: "Woops! You've lost all of the balls…😢 Change the properties and try again!")
    }
    ...
    func clear() {
        ...
        PlaygroundPage.current.assessmentStatus = .pass(message: "**Good Job! 🎉**. Why don't you  change properties and play again? I hope you enjoyed this AR Breakout.")
    }
    
    public func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        if let ball = BallNode.ball {
            ball.physicsBody?.velocity = ball.physicsBody!.velocity.normalized() * ballSpeed
